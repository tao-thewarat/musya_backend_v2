...
def run_multi_pipeline(
    ...
    code_result = _run_agent(
        ...
        (
            ...
            "l. ปัดตัวเลขก่อน print โดย **แยกชนิดคอลัมน์** (ห้าม df_display.round(2) ทั้งก้อน):\n"
            "   - คอลัมน์ตัวนับ (จำนวน|ราย|คน|ครั้ง|แห่ง|ผู้ป่วย|ประชากร|เป้าหมาย|ผลงาน)\n"
            "     ⇒ df_display[c] = df_display[c].round(0).astype('Int64')  # จำนวนคนไม่มีทศนิยม\n"
            "   - คอลัมน์อัตราส่วน (ร้อยละ|%|อัตรา|ค่าเฉลี่ย|สัดส่วน) ⇒ df_display[c] = df_display[c].round(2)\n\n"
