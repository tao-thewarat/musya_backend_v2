...
def run_pipeline(
    ...
    code_result = _run_agent(
        ...
        (
            ...
            "11. ปัดตัวเลขก่อน print โดย **แยกชนิดคอลัมน์** (ห้าม df_display.round(2) ทั้งก้อน):\n"
            "    - คอลัมน์ตัวนับ (ชื่อมี จำนวน|ราย|คน|ครั้ง|แห่ง|ผู้ป่วย|ประชากร|เป้าหมาย|ผลงาน)\n"
            "      ⇒ df_display[c] = df_display[c].round(0).astype('Int64')  # จำนวนคนไม่มีทศนิยม\n"
            "    - คอลัมน์อัตราส่วน (ชื่อมี ร้อยละ|%|อัตรา|ค่าเฉลี่ย|สัดส่วน)\n"
            "      ⇒ df_display[c] = df_display[c].round(2)\n"
            "    ⚠️ เคยหลุดออกไปเป็นคำตอบจริงมาแล้ว: 'ผู้ป่วยเบาหวานขึ้นทะเบียน 45,231.00 คน'\n"
            "Wrap code in ```python ... ```\n\n"
    ...
    insight = _run_agent(
        ...
        (
            ...
            "  • ตัวเลขร้อยละ/อัตราแสดง 2 ทศนิยม พร้อมหน่วย (%) — "
            "แต่จำนวนคน/ราย/ครั้ง ต้องเป็นจำนวนเต็มพร้อมคั่นหลักพัน ห้ามมี .00\n"
